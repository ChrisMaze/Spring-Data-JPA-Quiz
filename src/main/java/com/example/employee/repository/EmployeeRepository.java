...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    String findEmployeeByCompanyId(Integer companyId);
    //4.实现对Employee的分页查询，每页两个数据
    @Override
    Page<Employee> findAll(Pageable pageable);
    //5.查找**的所在的公司的公司名称
    @Query("select c.companyName from Company c where c.id = (select e.id from Employee e where e.name = ?1)")
    String findCompanyNameByEmplpyeeName(String employeeName);
